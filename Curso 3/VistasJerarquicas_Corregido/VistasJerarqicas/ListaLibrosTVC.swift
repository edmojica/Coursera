...
//  VistasJerarqicas
//
//  Created by Edson Mojica on 16/9/16.
...
class ListaLibrosTVC: UITableViewController {
    ...
    */
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        ...
        let libroVC = segue.destinationViewController as! LibroVC
        libroVC.cIsbn = nil
        if let ip = self.vistaLibros.indexPathForSelectedRow {
            libroVC.cIsbn = self.libros[ip.row][1]
        }
    }
    ...
    @IBAction func unwindToList(segue: UIStoryboardSegue) {
        ...
        let libroVC = segue.sourceViewController as! LibroVC
        print("return \(libroVC.cISBN.text!)")
        if (libroVC.cISBN.text! != "" ) {
            self.libros.append([libroVC.titulo.text!,libroVC.cISBN.text!])
        }
        self.vistaLibros.reloadData()
    }
